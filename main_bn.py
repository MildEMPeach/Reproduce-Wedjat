...
class Model():
    ...
    def predict(self, onesample: List[np.ndarray]): # 写一个算法，输入一个样本，输出预测的结果
        ...
        print("right ratio:",cnt_predict_right/(cnt_predict_right+cnt_predict_wrong))
        # print("posterior probs:", all_posterior_prob)
        # input()1
        # print("推断时间:", infertime)
...
if __name__ == "__main__":
    ...
    for onebag in tqdm(lst_white_bags[:], desc="构造训练数据"):
        ...
        for each_flow in feats_flows:
            ...
            # for p_i in range(len(output)):
            #     p_c = output[p_i]
            #     pi_label_coarse = label_each_cluster[p_c]
                
            #     dis_p_to_center = distances_to_centers[p_i][p_c]
            #     all_dis_to_center = all_distances_trainset[:,p_c]
            #     similar_p_c = sum(all_dis_to_center>dis_p_to_center)/len(all_dis_to_center)
            #     if pi_label_coarse == BLACK:
            #         output[p_i] = -1 * similar_p_c# round(-1*similar_p_c,1)
            #     else:
            #         output[p_i] =similar_p_c # round(similar_p_c,1)
            # print(output)
